...
if [[ "x$STIG_DATABASE" == "xmysql" ]]; then
	...
	for p in `mysql_query "SELECT 1 FROM mysql.user WHERE User=''"`; do
		...
		if [[ "x$p" == "x1" ]]; then
			...
			echo '<result>fail</result><message>Anonymous user has database privileges</message>'
			exit
		fi
	done
	...
	for p in `mysql_query "SELECT 1 FROM mysql.tables_priv WHERE User=''"`; do
		...
		if [[ "x$p" == "x1" ]]; then
			#echo anonymous username
			echo '<result>fail</result><message>Anonymous user has table privileges</message>'
			exit
		fi
	done
	...
	for p in `mysql_query "SELECT 1 FROM mysql.columns_priv WHERE User=''"`; do
		...
		if [[ "x$p" == "x1" ]]; then
			#echo anonymous username
			echo '<result>fail</result><message>Anonymous user has column privileges</message>'
			exit
		fi
	done
