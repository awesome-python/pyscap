...
class ocil(Model):
    ...
    def parse_sub_el(self, sub_el):
        if sub_el.tag == '{http://scap.nist.gov/schema/ocil/2.0}document':
            self.document = Model.load_child(self, sub_el)
        elif sub_el.tag == '{http://scap.nist.gov/schema/ocil/2.0}questionnaires':
            for sub_sub_el in sub_el:
                self.questionnaires[sub_sub_el.attrib['id']] = Model.load_child(self, sub_sub_el)
        elif sub_el.tag == '{http://scap.nist.gov/schema/ocil/2.0}test_actions':
            for sub_sub_el in sub_el:
                self.test_actions[sub_sub_el.attrib['id']] = Model.load_child(self, sub_sub_el)
        elif sub_el.tag == '{http://scap.nist.gov/schema/ocil/2.0}questions':
            for sub_sub_el in sub_el:
                self.questions[sub_sub_el.attrib['id']] = Model.load_child(self, sub_sub_el)
        elif sub_el.tag == '{http://scap.nist.gov/schema/ocil/2.0}artifacts':
            for sub_sub_el in sub_el:
                self.artifacts[sub_sub_el.attrib['id']] = Model.load_child(self, sub_sub_el)
        elif sub_el.tag == '{http://scap.nist.gov/schema/ocil/2.0}variables':
            for sub_sub_el in sub_el:
                self.variables[sub_sub_el.attrib['id']] = Model.load_child(self, sub_sub_el)
        elif sub_el.tag == '{http://scap.nist.gov/schema/ocil/2.0}results':
            self.results = Model.load_child(self, sub_el)
        else:
